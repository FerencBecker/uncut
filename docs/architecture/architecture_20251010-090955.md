...
```
backend/BffMini/
├── BffMini.csproj                   # .NET 9.0 project file with minimal dependencies
...
- **Stage 1 (Build)**: Uses microsoft/dotnet/sdk:9.0 with --platform=$BUILDPLATFORM for native-speed compilation
- **Stage 2 (Runtime)**: Uses microsoft/dotnet/aspnet:9.0 runtime-only image for minimal attack surface (225MB amd64, 256MB arm64)
