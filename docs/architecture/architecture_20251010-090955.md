...
**Screensaver Animation Patterns** (`frontend/src/components/Screensaver/Screensaver.tsx`):
- **Continuous Loop Animation**: `useAnimation` hook manages infinite animation cycles
- **Batched Entrance**: Studio markers animate in groups of 5 with 0.5s delay between batches for performance
- **Animation Cycle**: Each marker animates over 6s (scale 0→1.3→1), followed by 2s pause before restart
- **Key-based Remount**: Incrementing `animationKey` prop forces React remount for fresh animation cycles
- **Performance Optimization**: Batched animations reduce simultaneous renders from N markers to N/5 batches
- **GPU Acceleration**: Dark gradient background (#1A1A1A→#2C2C2C) with 30s rotating radial overlay using transform
- **Responsive Scaling**: 1.25x scaling for 55-inch displays (1920x1080+) via CSS media query

**Key Decision: Batched vs Sequential Animations**
- Rationale: 100 markers with sequential 0.3s delays = 30s total duration, causing extended idle periods
- Solution: Batch size of 5 markers animating simultaneously reduces total duration to ~10s while maintaining visual effect
- Trade-off: Slightly increased render load per batch vs much shorter animation cycle
- Benefit: Better engagement for kiosk screensaver mode with more frequent animation restarts
...
**Components**:
- **ErrorBoundary**: React error boundary with automatic retry, correlation IDs (`err-{timestamp}-{random}`), Hungarian/English UI
- **withErrorHandling**: Event handler wrapper catching errors ErrorBoundary misses (onClick, onTouch, async handlers)
- **usePerformanceMonitoring**: Hook tracking FPS (requestAnimationFrame), touch response (<200ms target), memory (Chrome API)
- **PerformanceMonitor**: Development-only dashboard (import.meta.env.DEV)

**Error Coverage**: Two-layer approach required because ErrorBoundary only catches render/lifecycle errors, not event handler errors. All onClick/onTouch handlers wrapped with withErrorHandling for complete coverage.

**Key Decision**: Hook-based monitors (useFPSMonitoring, useMemoryMonitoring, useTouchResponseTracking) using React primitives (useState, useRef) for state management. Avoids OOP indirection while maintaining requestAnimationFrame control via useRef for animation frame IDs.

## 11. Page-Level Integration (Issue #69)

**Problem**: Map components (HungaryMap, Screensaver) exist but no kiosk app orchestrates navigation, backend API, and user flow.

**Solution**: Created page-level wrappers (`pages/Screensaver`, `pages/Map`) with state-based navigation in App.tsx. Environment variable (`VITE_KIOSK_MODE`) toggles between kiosk and demo modes.

**Architecture**:
- **Pages**: Presentation-only components in `src/pages/` wrapping map components
  - `Screensaver.tsx`: Fullscreen overlay with "touch to begin" CTA, wraps ScreensaverComponent
  - `Map.tsx`: Header with theme/language toggles, map container with loading/error states, wraps HungaryMap
- **Navigation**: State-based in KioskApp (`'screensaver' | 'map'`), no React Router (YAGNI - only 2 pages)
- **API Integration**: KioskApp fetches studios via `api.studios.getAll()`, passes down to pages
- **Inactivity Timer**: `useInactivityTimer` hook monitors events (mousedown, keydown, touchstart, scroll), returns to screensaver after 2 minutes
- **Mode Switching**: App.tsx routes to KioskApp or DemoApp based on `VITE_KIOSK_MODE`

**Naming Convention**: Pages in `pages/` folder named without "Page" suffix (e.g., `Screensaver.tsx` not `ScreensaverPage.tsx`). Import aliases used in App.tsx (`ScreensaverPage`, `MapPage`) to avoid conflicts with component-level imports.

**Key Decision**: No routing library needed for kiosk with 2 pages. Simple state management keeps complexity minimal. Error handling uses fallback empty arrays to keep app functional when backend unavailable.
